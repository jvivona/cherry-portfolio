...
class Cherry_Portfolio_Data {
	...
	public function get_portfolio_items_loop( $posts_query, $listing_layout = 'masonry-layout', $template = 'masonry-default.tmpl' ) {
		...
		$output = '';
			...
			if ( $posts_query->have_posts() ) {
				...
				if ( 'justified-layout' === $listing_layout ) {
	...
	public function build_single_post_page( $post_id, $format ) {
		...
		// Get category list.
		$taxonomy_name_list = ( 'category' == $tax[0] ) ? $this->get_taxonomy_list( $post_id, 'category' ) : $this->get_taxonomy_list( $post_id, 'tag' );
		...
		$html .= '<div class="portfolio-filter with-ajax" data-order-default="' . $options['order_filter_default'] . '" data-orderby-default="' . $options['orderby_filter_default'] . '">';
			...
			if ( 'true' == self::$options['order_filter_visible'] ) {
				$html .= '<ul class="order-filter">';
					$class = ( 'asc' == $options['order_filter_default'] ) ? 'class="dropdown-state"' : '' ;
					...
					if ( $page_count > 1 ) {
						$dom_part .= '<ul class="page-link">';
							...
							for ( $i = 0; $i < $page_count; $i ++ ) {
								 ...
								 * @var string
								 */
								$before_pagination_item = apply_filters( 'cherry-portfolio-before-pagination-item', '', $counter );
								 ...
								 * @var string
								 */
								$after_pagination_item = apply_filters( 'cherry-portfolio-after-pagination-item', '', $counter );

								if ( $i !== $current_page_index - 1 ) {
									$dom_part .= sprintf( '<li><a href="javascript:void(0)">%1$s%2$s%3$s</a></li>', $before_pagination_item, $counter, $after_pagination_item );
								} else {
									$dom_part .= sprintf( '<li class="active"><a href="javascript:void(0)">%1$s%2$s%3$s</a></li>', $before_pagination_item, $counter, $after_pagination_item );
								}
							}
						$dom_part .= '</ul>';
...
function get_new_items() {
	if ( ! empty( $_POST )
		...
		( 'all' !== $value_slug ) ? $_POST['value_slug'] : $value_slug = '';

		$data = new Cherry_Portfolio_Data;

		$query_args = array(
			( 'category' == Cherry_Portfolio_Data::$default_options['filter_type'] ) ? CHERRY_PORTFOLIO_NAME . '_category' : CHERRY_PORTFOLIO_NAME . '_tag' => $value_slug,
