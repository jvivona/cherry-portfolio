	...
	class Cherry_Page_Builder {
		...
		*/
		...
		public function add_child_menu_item( $attr ){
			...
			$prefix = ( strpos( $attr[ 'parent_slug' ], 'post_type' ) ) ? '&page=' : $prefix ;
		...
		public static function build_page(){
			...
			if( array_key_exists( $page_hook , self::$pages[ 'parent' ] ) ){
				$page_attr = self::$pages[ 'parent' ][ $page_hook ];
			}elseif( array_key_exists( $page_hook , self::$pages[ 'child' ] ) ){
				$page_attr = self::$pages[ 'child' ][ $page_hook ];
			}else{
				$page_attr = self::$pages[ 'child' ][ 'edit.php?' . $_SERVER['QUERY_STRING'] ];
			}
 ...
 * @return object
 */
function cherry_page_builder() {
	return Cherry_Page_Builder::get_instance();
}
